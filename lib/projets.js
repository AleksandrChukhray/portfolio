...
export function getBGColor(str) {
    ...
    let numPair = [];
    let maxNum = 0;
    let strPairLength = Math.ceil(strLength / 6);

    do {
        let pushStr = newStr.substr(start, strPairLength);

        if(!pushStr) pushStr = Array(strPairLength).fill('a').join();
    ...
    if(strPair.length < 6){
        while(strPair.length < 6){
            let s = Array(strPairLength).fill('a').join();
            strPair.push(s)
        }
    }
    ...
    numPair.forEach((value, index) => {
        let hex = Math.floor((value * 15) / maxNum).toString(16);
